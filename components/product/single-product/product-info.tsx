...
function ProductInfo({ product, categoryPath }: { product: SingleProductDTO; categoryPath: string[] }) {
  const stock = getProductStatusVariant(product.stock, product.lowStockThreshold)

  return (
    <section>
      <div className="mb-3 rounded-md bg-foreground/70 flex items-center w-fit">{categoryPath.map((cat, i) =>
        <button key={i} className="py-1 px-2 text-[12px] text-white flex gap-1 hover:bg-foreground/80 cursor-pointer duration-150 rounded-md items-center">
          <span>{cat}</span>
      ...
      <div className="flex gap-2 flex-wrap justify-between items-center my-5">
        ...
        <div className="flex gap-2 items-end font-semibold">
          ...
          <span className="text-3xl text-primary">{product.price}$</span>
          {
            ...
            <span className="text-xl line-through text-muted-foreground">{product.compareAtPrice}$</span>
          }
        </div>
        {
      ...
      {
        product.isCarted ?
          <Button variant="outline" className="flex gap-2 items-center justify-center w-full my-6">
